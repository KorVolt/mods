...
package net.mcreator.kormod.item.inventory;
...
public class BackpackInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == KormodModItems.BACKPACK.get()) {
			if (Minecraft.getInstance().screen instanceof CoffreportableScreen) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(49) {
			@Override
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != KormodModItems.BACKPACK.get();
			}
